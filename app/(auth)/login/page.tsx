...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'שגיאת כניסה');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'שגיאת כניסה עם Google');
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 mt-2 text-sm">כניסה לחשבון</p>
        </div>
        ...
        <p className="text-center text-sm text-gray-500 mt-6">
          אין לך חשבון?{' '}
          <Link href="/signup" className="text-indigo-400 hover:text-indigo-300">הרשמה חינם</Link>
        </p>
      </div>
    </div>
  );
